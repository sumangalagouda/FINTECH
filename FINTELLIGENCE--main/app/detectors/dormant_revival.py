...
DORMANCY_THRESHOLD_DAYS = 180
HIGH_DORMANCY_THRESHOLD_DAYS = 365
RAPID_ACTIVITY_WINDOW_DAYS = 7
RAPID_ACTIVITY_THRESHOLD = 3
...
def detect_dormant_revival(case_id: str, statement_id: str = None):
    ...
    for account, group in df.groupby("account"):
        ...
        for idx, row in group.iterrows():
            ...
            score = 40
            reasons = [
                f"Account remained inactive for {days_gap} days."
            ]
            ...
            if days_gap >= HIGH_DORMANCY_THRESHOLD_DAYS:
                ...
                reasons.append(
                    "Dormancy exceeded one year."
                )

            # --------------------------------------------------------------
            # Revival amount anomaly
            # --------------------------------------------------------------
            ...
            if pd.notna(hist_avg) and hist_avg > 0:
                ...
                if multiplier >= 3:
                    ...
                    reasons.append(
                        f"Revival transaction was "
                        f"{multiplier:.1f}x the historical average."
                    )
            ...
            if not subsequent.empty:
                ...
                rapid_txns = subsequent[
                    (
                        subsequent["date"] - row["date"]
                    ).dt.days
                    <= RAPID_ACTIVITY_WINDOW_DAYS
                ]

                if len(rapid_txns) >= RAPID_ACTIVITY_THRESHOLD:

                    score += 20

                    reasons.append(
                        f"{len(rapid_txns)} additional "
                        f"transactions occurred within "
                        f"{RAPID_ACTIVITY_WINDOW_DAYS} days."
                    )
    ...
    if not results:
        ...
        return [{
            ...
            "score": 0,
            "reason": (
                "No dormancy patterns detected within "
